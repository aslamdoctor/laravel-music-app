...
<h1 class="mt-10 text-2xl font-bold mb-5">Genre - Edit</h1>

<form method="post" action="/genre/{{ $genre->id }}" class="mb-5">
    @csrf
    @method('put')
    ...
    <div class="mb-5">
        ...
        @error('description') border-red-500 @enderror">{{ old('description', $genre->description) }}</textarea>
        @error('description')
            <div class="text-red-500">{{ $message }}</div>
        @enderror
    </div>
